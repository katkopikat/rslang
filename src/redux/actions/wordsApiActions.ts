...
const composeUrl = (userId: string, group: number, page: number, filter = {}) => {
...
const fetchForAnonTextbook = (group: number, page: number) => async (dispatch: any) => {
...
const fetchForUserTextbook = (group: number, page: number, userId: string, token: string) => (
  async (dispatch: any) => {
    const filterQuery = encodeURIComponent(JSON.stringify({ page, ...wordFilters.excludeDeleted }));
...
const fetchFiltered = (group: number, page: number, userId: string, token: string, filter = {}) => (
  async (dispatch: any) => {
    ...
    dispatch(setPaginationCount(Math.ceil((wordsTotal?.count || 0) / WORDS_PER_PAGE)));
  }
);

const fetchDeleted = (group: number, page: number, userId: string, token: string) => (
  fetchFiltered(group, page, userId, token, wordFilters.onlyDeleted)
);

const fetchDifficult = (group: number, page: number, userId: string, token: string) => (
  fetchFiltered(group, page, userId, token, wordFilters.onlyDifficult)
);

const fetchLearning = (group: number, page: number, userId: string, token: string) => (
  fetchFiltered(group, page, userId, token, wordFilters.learning)
);

const fetchFilteredCounts = (userId: string, token: string) => (
  async (dispatch: any, getState: any) => {
    const { app: { group } } = getState();
